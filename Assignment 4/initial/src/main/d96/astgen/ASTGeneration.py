...
class ASTGeneration(D96Visitor):
    ...
    def visitThe_rest_attributes_methods_declarations(self,
                                                      ctx: D96Parser.The_rest_attributes_methods_declarationsContext):
        memberList: List[MemDecl] = []
        if (ctx.attribute_declaration()):
            memberList = memberList + self.visit(ctx.attribute_declaration())
            return memberList + self.visit(ctx.the_rest_attributes_methods_declarations())
    ...
    def visitAttribute_declaration(self, ctx: D96Parser.Attribute_declarationContext):
        ...
        constList = [(name, mytype, value)
                     for name, value in zip(attribList, expList)]
        if ctx.VAL():
    ...
    def visitMethod_declaration(self, ctx: D96Parser.Method_declarationContext):
        if ctx.getChildCount() == 5:
            kind = Static() if ctx.getChild(0).getText()[
                0] == '$' else Instance()
    ...
    def visitIf_statements(self, ctx: D96Parser.If_statementsContext):
        ...
        if ctx.else_ifList():
            ...
            initValue = self.visit(ctx.else_statements()
                                   ) if ctx.else_statements() else None
            ...
            elseStmt = self.visit(ctx.else_statements()
                                  ) if ctx.else_statements() else None
    ...
    def visitForeach_statements(self, ctx: D96Parser.Foreach_statementsContext):
        ...
        expr3 = self.visit(ctx.exp(2)) if len(
            ctx.exp()) == 3 else IntLiteral(1)
        # NOTE: A single statement or a block
    ...
    def visitVariable_constant_declaration(self, ctx: D96Parser.Variable_constant_declarationContext):
        ...
        if ctx.VAL():
            if ctx.expList():
                ...
                return [ConstDecl(id, mytype, None) for id in idList] if isinstance(mytype, ClassType) else [
                    ConstDecl(id, mytype, None) for id in idList]
        elif ctx.VAR():
            if ctx.expList():
                expList = self.visit(ctx.expList())
                assert len(expList) == len(idList), "Not enough expressions"
                ...
                return [VarDecl(id, mytype, NullLiteral()) for id in idList] if isinstance(mytype, ClassType) else [
                    VarDecl(id, mytype, None) for id in idList]
