...
class CodeGenVisitor(BaseVisitor, Utils):
    ...
    def visitAssign(self, ctx, param):
        ...
        result = rhs[0] + self.visit(ctx.lhs, Access(param.frame, param.sym, True))[0]
    ...
    def visitFuncDecl(self,ctx,o):
        currFrame = o.frame
        currFrame.enterScope(True)
        startLabel = currFrame.getStartLabel()
        endLabel = currFrame.getEndLabel()
        param = ctx.param
        returnType = ctx.returnType
        paramType = [x.typ for x in param]
        mType = MType(paramType, returnType)
        self.emit.printout(self.emit.emitMETHOD(ctx.name, mType, True, o.frame))
        for eachParam in param:
            self.emit.printout(self.emit.emitVAR(currFrame.getNewIndex() -1, eachParam.name, 
            eachParam.typ, startLabel, endLabel, currFrame))
            self.visit()
        self.emit.printout(self.emit.emitLABEL(startLabel,currFrame))
